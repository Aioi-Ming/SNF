...
class ParticleDimer(object):
    ...
    def __init__(self, params=None, nsolvent= None):
        # set parameters
        if params is None:
            params = self.__class__.params_default
        self.params = params
        if nsolvent is not None:
            self.params['nsolvent']=nsolvent
    ...
    def draw_config(self, x, axis=None, dimercolor='blue', alpha=0.7):
        ...
        axis.add_patch(Rectangle((-d - self.params['rm'], -d - self.params['rm']),
                                 2 * d + 2 * self.params['rm'], 0.5 * self.params['rm'], color='lightgrey',
                                 linewidth=0))
        axis.add_patch(Rectangle((-d - self.params['rm'], d + 0.5 * self.params['rm']),
                                 2 * d + 2 * self.params['rm'], 0.5 * self.params['rm'], color='lightgrey',
                                 linewidth=0))
        axis.add_patch(Rectangle((-d - self.params['rm'], -d - self.params['rm']),
                                 0.5 * self.params['rm'], 2 * d + 2 * self.params['rm'], color='lightgrey',
                                 linewidth=0))
    ...
    def LJ_energy_torch(self, x):
        ...
        D2rel = D2rel * mmatrix
        # energy
        E = 0.5 * self.params['eps'] * torch.sum(D2rel ** 6,
                                                 axis=(1, 2))  # do 1/2 because we have double-counted each interaction
        return E

    def dimer_energy_distance(self, d):
        d0 = (d - self.params['dimer_dmid'])
        d2 = d0 * d0
        d4 = d2 * d2
        energy_interaction = self.params['dimer_slope'] * d0 - self.params['dimer_b'] * d2 /2 + self.params['dimer_a'] * d4 /4
    ...
    def dimer_energy(self, x):
        ...
        # first two particles
        d= self.dimer_distance(x)
        energy_interaction = self.dimer_energy_distance(d)
    ...
    def energy_torch(self, x):
        return self.LJ_energy_torch(x) + self.dimer_energy_torch(x) + self.box_energy_torch(x)
